...
<template>
  <div class="bg-gray-100 h-full ">
    ...
    <!--        </SelectItem>-->
    <!--        <SelectItem-->
    <!--          v-if="filterData.areaList.length"-->
    <!--          title="请选择学期"-->
    <!--        >-->
    <!--          <el-radio-group-->
    <!--            v-model="filterData.examName"-->
    ...
    <!--            v-model="filterData.grade"-->
    <!--            :disabled="!filterData.schoolDistrict"-->
    <!--            class="p-4"-->
    <!--            @change="changeExam"-->
    <!--          >-->
    <!--            <el-radio-->
    ...
    <div
      v-if="barData"
      class="bg-white rounded-md mt-4 px-4 rounded-xs"
    >
      <div
...
import { getAvg, getExamName, getProject, getScore } from "@/api/dataOverview";
...
let filterData = reactive({ ...defaultData });
...
watch(() => [filterData.gender, filterData.grade, filterData.project, filterData.examName], () => {
    totalScoreChartData.value = null;
    projectScoreChartData.value = null;
...
const changeProject = (name: any) => {
  projectName.value = projects.value.find((item: any) => item.value == name).name;
  // submitFilterData()
};
const changeSelectCity = (value) => {
  projectName.value = filterData.examName = "";
  filterData.gender = filterData.grade = undefined;
  filterData.areaList = [];
  // filterData = reactive(efaultData);
...
const changeSchoolList = (value) => {
  ...
  filterData.grade = "";
  filterData.examName = "";
  if (filterData.areaList.length)
    getExamList();
};
const getExamList = (flag = true) => {
  if (!flag) return;
  filterData.examName = "";
...
const changeExam = async () => {
  ...
  return await getProject(parmeter).then((res) => {
...
const handleClick = (tab) => {
  ...
  if (tab.index == "0") {
    ...
    if (projectScoreChartData.value || !filterData.project) return;
    getAvgScore();
  }
};
const getAllScore = () => {
  filterData.areaList = checkSchoolList.value;
  getScore(filterData).then((res) => {
