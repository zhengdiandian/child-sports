<template>
  ...
  <div class="p-8">
    ...
    <div>
      <h5 class="text-black text-2xl my4">
        转入新班级
      </h5>
      <SchoolSelect
        v-model:class-info="classInfo"
        :show-info="false"
        :show-all-class-names="false"
        :show-all-school-names="false"
        :showitems="['schoolNames', 'schoolType', 'className']"
      />
    </div>
    <div class=" w-full flex justify-center my-8 pr-10">
      <el-button
        type="primary"
        :disabled="!classInfo.id && !multipleSelection.length"
        class="w-36 h-10"
        @click="transferClickHandle"
      >
        批量转学
      </el-button>
    </div>
...
const transferClickHandle = () => {
  ElMessageBox.confirm(`确定要将${studentNames.value}转至${classInfo.value.name}`).then(() => {
    transferBatch({
      newClassId: classInfo.value.id,
      studentIds: multipleSelection.value.map((student: any) => student.studentId)
    })
      .then((res: any) => {
        if (res.code === 200) {
...
const studentNames = computed(() => multipleSelection.value.map((student: any) => student.studentName).toString());
